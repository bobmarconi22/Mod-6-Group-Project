    ...
    (parseInt(id) === shopDetails?.id && (
      <>
        <div id="shop-detail-cover-container">
          <img
            src={
              shopDetails?.image.find((img) => img.preview_image === true)
                ?.img_link
            }
            id="prev-img"
          />
          <div className="shop-detail-heading-container">
            ...
            <div id="rating-container">
              ...
              <div id="price-categories-container">
                ...
                <div id="categories">
                  {shopDetails?.categories?.map((category, i) => {
                    return (
                      <div
                        key={i}
                        ...
                        onClick={() =>
                          navigate(
                            `/search?category=${category.split(" ").join("+")}`
                          )
                        }
                      >
                        {category}
            ...
            <div id="additional-details">
              <a target="_blank" href={shopDetails?.website}>
                {shopDetails?.website}</a> {shopDetails?.phone_number}
              <p>{shopDetails.address.address_line1
            ? shopDetails.address.address_line1
            : ""}{" "}
          {shopDetails.address.address_line2
            ? shopDetails.address.address_line2
            : ""}
          {shopDetails.address.city ? shopDetails.address.city : ""},{" "}
          {shopDetails.address.state ? shopDetails.address.state : ""}{" "}
          {shopDetails.address.postal_code
            ? shopDetails.address.postal_code
            : ""}{" "}
          {shopDetails.address.country ? shopDetails.address.country : ""}</p>
            </div>
          </div>
          <Link to={`images`}>
            <button className="see-all-photos-button">
              See all #{shopDetails?.image?.length} photos
            </button>
          </Link>
          {/* shop details is checked before it gets the information, must check if shopdetails exists */}
          {sessionUser &&
          shopDetails &&
          sessionUser.id === shopDetails.owner_id ? (
        ...
        <div id="about-section">
          About {shopDetails?.name}:<div>{shopDetails?.description}</div>
        </div>

        <div className="additional-info-flexbox-container">
          <div>
            {shopDetails?.hours?.monday === "Closed" &&
            shopDetails?.hours?.tuesday === "Closed" &&
            shopDetails?.hours?.wednesday === "Closed" &&
            shopDetails?.hours?.thursday === "Closed" &&
            shopDetails?.hours?.friday === "Closed" &&
            shopDetails?.hours?.saturday === "Closed" &&
            shopDetails?.hours?.sunday === "Closed" ? (
              <>No Hours Specified For this Café</>
            ) : (
              <div
                className={
                  showHours ? "hours-container" : "hours-container-hidden"
                }
              >
                <p>All Hours</p>
                ...
                <button onClick={() => setShowHours((prevShow) => !prevShow)}>
                  Hours
                </button>
              </div>
            )}
          </div>
          <div id="shop-detail-additional-info-container"></div>
        </div>
        <ShopDetailsReviews
          rating={shopDetails?.avg_rating}
          shop={shopDetails}
        />
      </>
    ))
